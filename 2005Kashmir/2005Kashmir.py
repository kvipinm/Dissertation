...
try:
    ...
    data=np.fromfile("./loc/global.20051008.035053.grid0.loc.scat", dtype=dt)
...
with fig1.inset(position="jBR+w3c/3c+o0.1c", box="+gwhite+p1p"):
    fig1.coast(
        region=[region[0]-0.2, region[1]+2.3, region[2]-2.1, region[3]+0.3],
        projection="M3c",
        land="gray",
        borders=[1, 2],
        shorelines="1/thin",
    ...
    rectangle = [[region[0], region[2], region[1], region[3]-0.1]]
    fig1.plot(data=rectangle, projection="M3c", style="r+s", pen="1p,red")

fig1.savefig('2005Kashmir_pdf_samples.png')
fig1.show()
...
fig2.savefig('2005Kashmir_stations.png')
fig2.show()
...
plt.savefig('2005Kashmir_depth_prob.png', bbox_inches='tight')
plt.show()
